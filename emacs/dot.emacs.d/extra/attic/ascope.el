...
;; This command must be issued prior to issue any other command below,
;; the directory you give to this command must be the include/ for the cscope.out file
...
;;	 ascope-next-symbol								(bound to key "n")
;;	 ascope-prev-symbol								(bound to key "p")
;;	 ascope-select-entry-other-window-delete-window (bound to key "enter")

;; Motivation:
;; Because xcscope.el open a new process of "cscope -L" for each a definition,
;; it's wasting time for reloading cscope and reopening database every time.
;; Another cscope interface from bscope.el can get the search result quick,
;; but has poor functionality.
;; This merges xcscope.el and bscope.el together to make it fast and versatile.
